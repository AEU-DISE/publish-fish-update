...
#updated 3-27-25 for 2024 update L.Vance
# updated 4-15-26 for 2025 update L.Vance
...
sample_effort <- left_join(event, effort) %>%
  select(-SeineLength, -SeineDepth, -SeineWidth, -RevCounter)

catch <- left_join(sample_effort, total_catch) %>%
  relocate(c(SeineVolume, TrapStatus, TrapHours, TotalRevs), .after = Count) %>%
...
To calculate CPUE (note units are different for BSEIN (catch/volume) vs traps (catch/hr))
```{r}
