...
##updated Dec 2024 for update through end of WY2024
## updated beginning 10/2025 for update through end of WY 2025
...
## 2. Read in file
```{r importdata}
# Data retrieved date noted in file name from Access 
...
effort <- read.csv("data_raw/YBFMP_TrapHours_2010_2025.csv")
fish_stations <- read.csv("data_raw/YBFMP_Stations_Coordinates_Info_20250219.csv")
fish_ref <- read.csv("data_raw/Sample_20260310.csv")
fish_tax <- read.csv("data_raw/OrganismsLookUp_20250214.csv") #updates to shrimp genus and lamprey genus
fish_tax_ed <- read.csv("data_raw/FishTaxonomyKISTERS.csv") #Edited for KISTERS, only has species present
smelt_gen <- read.csv("data_raw/SmeltGenetics_20250317.csv")
salm_gen <- read.csv("data_raw/SalmGenetics_20260324.csv")
salm_gen2 <- read.csv("data_raw/gemlabgenetics.csv")
seine <- read.csv("data_raw/SeineEffort_20260310.csv")
trap <- read.csv("data_raw/TrapEffort_20260310.csv")
...
fish_ref_clean <- fish_ref %>% 
  ...
  rename(WaterTemp = WaterTemperature,
         ...
         GearCode = GearID,
         EventID = SampleID)%>%
  filter(!(EventID == "6600" | EventID == "7749" | EventID == "4779" |
           EventID == "6861" | EventID == "6862" | EventID == "7533")) %>% #filters duplicated entries found in JTs beach seine analysis
  mutate(across(.cols = c(GearCode, WeatherCode), .fns = na_if, "")) %>%    #update this for updated dplyr function?
    filter(lubridate::year(Datetime) < 2026) %>%
  mutate(SampleAltered = ifelse(SampleDate<ymd("2025-10-01"), NA, SampleAltered)) %>%
  ...
  filter(Datetime<"2025-10-01")%>%
  arrange(Datetime)

#find and remove or correct station code PCS in seine data as warranted - discovered during scientific review
pcsseine <- fish_ref_clean %>%
  filter(StationCode == "PCS" & MethodCode == "BSEIN")
#dates in 1998/1999/2000 likely labeled correctly for early program

#2007 unclear - data sheet handwritten - remove entry due to lack of clarity
fish_ref_clean <- fish_ref_clean %>% 
  filter(!(EventID == 3914))
#2019 mislabled as pcs when is BL1 - Data sheet confirmed
fish_ref_clean$StationCode[fish_ref_clean$EventID == 8396] <- "BL1"

#find sw2, should be SB2, and fix (from scientific review)
sw2 <- fish_ref_clean %>% 
  filter(StationCode == "SW2" & MethodCode == "BSEIN")
fish_ref_clean$StationCode[fish_ref_clean$EventID == 10119] <- "SB2"

cc <- fish_ref_clean %>% 
  filter(StationCode == "CC27")
fish_ref_clean$StationCode[fish_ref_clean$EventID == 10608] <- "CCS7"

#find and correct gear code for 3 dates found during sci review
gearfix <- fish_ref_clean %>% 
  filter(GearCode == "FKTR" & MethodCode == "RSTR")
fish_ref_clean$GearCode[fish_ref_clean$EventID == 2415] <- "RSTR8"
fish_ref_clean$GearCode[fish_ref_clean$EventID == 3469] <- "RSTR8"
fish_ref_clean$GearCode[fish_ref_clean$EventID == 3619] <- "RSTR8"
fish_ref_clean$GearCode[fish_ref_clean$EventID == 4081] <- "RSTR8"
#additional gear fixes
fish_ref_clean$GearCode[fish_ref_clean$EventID == 10648] <- "RSTR8"
fish_ref_clean$MethodCode[fish_ref_clean$EventID == 10648] <- "RSTR"
fish_ref_clean$GearCode[fish_ref_clean$EventID == 10643] <- "FKTR"
fish_ref_clean$MethodCode[fish_ref_clean$EventID == 10643] <- "FKTR"
...
fish_catch_clean0 <- fish_catch %>%
  ...
  rename(FishIDComments = Comments,
         ...
         RaceByLength = Race,
         EventID = SampleID) %>%
  filter(!(EventID == "6600" | EventID == "7749")) %>%   #removes duplicate entries found in JTs beach seine analysis
...
unique(fish_catch_clean$Expression) ##updated for renamed column spawningcondition -> expression
...
#correct gen tag numbers that were mistyped
fish_catch_clean$FishGenID[fish_catch_clean$EventID == 10348 & fish_catch_clean$ForkLength == 830] <- "2023_ACHN_001"
fish_catch_clean$FishGenID[fish_catch_clean$EventID == 10350 & fish_catch_clean$ForkLength == 762] <- "2023_ACHN_002"
fish_catch_clean$FishGenID[fish_catch_clean$EventID == 10354 & fish_catch_clean$ForkLength == 798] <- "2023_ACHN_003"
fish_catch_clean$FishGenID[fish_catch_clean$EventID == 10355 & fish_catch_clean$ForkLength == 761] <- "2023_ACHN_005"
fish_catch_clean$FishGenID[fish_catch_clean$EventID == 10355 & fish_catch_clean$ForkLength == 889] <- "2023_ACHN_004"
fish_catch_clean$FishGenID[fish_catch_clean$EventID == 9869 & fish_catch_clean$ForkLength == 775] <- "2022_ACHN_006"
fish_catch_clean$FishIDComments[fish_catch_clean$EventID == 9869 & fish_catch_clean$ForkLength == 775] <- "Floy Tag #470"

# find duplicated data discovered during beach seine data analysis for paper in progress
BL12016 <- filter(fish_catch_clean, EventID == "6760")
BL12016dup <- filter(fish_catch_clean, EventID == "6758") #no FL entered so this is duplicated entry (discovered in JTs beach seine analysis) - remove
BL52011 <- filter(fish_catch_clean, EventID == "4779") # seine was re-done - delete this data
BL52011dup <- filter(fish_catch_clean, EventID == "4780") #second seine done due to gear issue- keep this data (discovered in JTs beach seine analysis) 

BL52017 <- filter(fish_catch_clean, EventID == "7534") #first seine
BL52017dup <- filter(fish_catch_clean, EventID == "7533") #second seine done to catch CHN for diet study, remove per find in JTs beach seine analysis paper - remove this entry

March2016 <- filter(fish_catch_clean, EventID == "6861") #first of 3 seines done - delete record (beach seine analysis)
March2016B <- filter(fish_catch_clean, EventID == "6862") #second of 3 seines done - delete record (beach seine analysis)
March2016C <- filter(fish_catch_clean, EventID == "6863") # third seine done - keep this record

#remove the confirmed duplicate Event ID 
fish_catch_clean <- fish_catch_clean %>%
  filter(!(EventID == "4779" | EventID == "6600" | EventID == "6861" |
             EventID == "6862" | EventID == "7533" | EventID == "7749"))

#all duplicated instances discovered in beach seine analysis removed
...
fish_tax_clean <- fish_tax %>%
  ...
  mutate(Species = replace(Species, OrganismCode == "MSS", "audens"),
         Genus = replace(Genus, OrganismCode == "KLF", "Lucania"))
...
seine_clean <- seine %>%
  ...
  mutate(across(.cols = c(HabitatType, Tide_Seine), .fns = na_if, "")) %>%
    filter(!(EventID == "4779" | EventID == "6600" | EventID == "6861" |
             EventID == "6862" | EventID == "7533" | EventID == "7749")) # removes duplicated entries found in beach seine analysis
...
```{r}
# Trap
trap_clean <- left_join(fish_ref_clean %>% select(StationCode, MethodCode,SampleDate, Datetime, EventID),trap %>% select(EventID = SampleID, Comments, TrapStatus, Tide, Rev.Counter)) %>%
  mutate(effort.hrs = NA,
         TotalRevs = NA) %>%
  rename(TrapComments = Comments,
         EffortHours = effort.hrs,
         RevCounter = Rev.Counter) %>%
  select(StationCode, MethodCode,SampleDate, Datetime, EventID, TrapStatus, Tide, EffortHours, RevCounter, TotalRevs, TrapComments) %>%
...
smelt_gen_clean <- smelt_gen %>%
  ...
  select(SmeltGeneticRowID, FishGenID, qDSM, qWAG, qLFS, GeneticID, VShape, Comments_Smelt) 
...
## clean up and add GEM lab genetics to file
salm3 <- salm_gen2 %>% 
...
salm_gen_clean <- bind_rows(salm_gen_clean, salm3)
...
# 7641, 8050, 9261, 9505, 9515, 10558 - screw trap not set (copied over from effort file)
# 10746 was check date (copied over from effort file)
...
# 10137, 10161, 10225, 10257, 10348, 10367, 11129
...
Sample_effort_seine <- left_join(Sample_effort, seine_clean_qc) %>%
 ...
 # mutate_if(is.character, empty_as_na) %>%
  ...
  arrange(MethodCode)%>%
  mutate(StationCode = case_when(StationCode == "RD22" ~ "CCS7",
                                 TRUE ~ StationCode)) %>%
  select(-c(ConditionCode, TrapComments, SeineComments, Tide_Trap, Tide_Seine))
#renamed RD22 to CCS7 because the sites are actually two different locations, RD22 is 
# only used for NDFS/lower trophic
  
#check that sites were renamed correctly  
CCS7 <- filter(Sample_effort_seine, StationCode == "CCS7")
RD22 <- filter(Sample_effort_seine, StationCode == "RD22")
...
check2022 <- filter(SampleComments, year(SampleDate) == 2022 & FieldComments!="")
check2023 <- filter(SampleComments, year(SampleDate) == 2023 & FieldComments!="")
check2024 <- filter(SampleComments, year(SampleDate) == 2024 & FieldComments!="")
check2025 <- filter(SampleComments, year(SampleDate) == 2025 & FieldComments!="")
```
...
shouldbe4 <- c(4016, 4014, 4013, 4015, 4340, 4947, 5471, 5503, 5532, 8317, 6039, 6256, 6991, 9812, 9835, 9857, 9919, 10013, 10072, 10078, 10090, 10091, 10117, 10118)
...
shouldbe2 <- c(1524, 1525,1401, 2680, 4228, 5948, 7587, 6865, 6109, 6849, 761, 762, 829, 835, 1086, 1113, 7561,
               #screw trap
               1401, 5279, 6540, 9908, 10106, 10280, 8529, 10616)
# no seine (hyacinth, too shallow, too deep, other veg): 4016, 4014, 4013, 4015, 4340, 4947, 5471, 5503, 5532, 6039, 6849 (check), 6256 (tide is too high??), 6991 (check), 9812, 9835, 9857, 9919, 10013, 10072, 10074, 10078, 10090, 10091, 10117, 10118, 10338, 10346
# 10616 was mis-recorded - weather code instead of condition code
# twisted bag: 5948, 7587, (2022/2023 finds correctly marked as cond code 2)
# flipped seine: 6109
# clogged cone/debris: 1401
# might have lost some catch: 2680
# fyke not set to full depth: 761, 762, 1113
# fish escape: 829, 835, 1086, 7561, 7564, 9908, 10106, 10280, 8529
```
...
9534, 9536, 9542, 9648, 9650, 9665, 9671, 9676, 9677, 9708, 9759, 9760, 9778, 9786, 9802, 9803, 9810, 9827, 9828, 9836, 9837, 9850, 9851, 9912, 9913, 9924, 9925, 9926, 9943, 9944, 9949, 9981, 9987, 10003, 10007, 10012, 10021, 10034, 10067,
10069, 10083, 10210, 10254, 10290, 10299, 10079, 9990, 9991, 10100, 10101, 10102, 10103, 10108, 10110, 10111, 10112, 10133,
# 10095, 10097, 10098, 10099, 10105, 10138, 10139, 10599 (overtopping, short sets due to flow/debris)
```
...
Effort_alone <- Sample_effort_clean %>%
  select(EventID, SeineLength, SeineWidth, SeineDepth, SeineVolume = Volume, TrapStatus, TrapHours = EffortHours, RevCounter, TotalRevs)
...
Other <- c("YB", "FW1", "SB1", "YBI80", "SB2", "LIHF", "LIHFS", "CCS7")
#removed RD22 - name changed confirmed
```
...
phtemp <- ggplot(fish_WQ_outliers3) + geom_point(aes(pH, WaterTemp, color = Region))
ggplotly(phtemp)
...
ggplotly(AL_outliers)

# Other
Other_outliers <- Outliers0 %>%
filter(Region == "Other") %>%
ggplot() + 
  geom_point(aes(Datetime, Value, colour = factor(Outlier)), size = 1) + 
  facet_wrap(~Parameter, scales = "free") + scale_colour_viridis(discrete = TRUE) + theme_bw() + labs(title = "Other")
ggplotly(Other_outliers)
```

```{r}
OutliersToCheck <- filter(Outliers0, Outlier >=1)

check2223 <- filter(OutliersToCheck, (Year == "2022" | Year == "2023"))
check24 <- filter(OutliersToCheck, (Year =="2024"))
check25 <- filter(OutliersToCheck, (Year == "2025"))
```
...
#make sure to leave cond code 3 in, leave cond code 4 out
# "duplicated" data - multiple seines/short set for screw trap etc using same WQ - find and add notation  - add column for duplicated/not dup

DuplicatedWQ <- fish_sample_cleaned %>% count(StationCode, SampleDate, WaterTemp, SpecificConductance, Conductivity, 
          Turbidity, DO, pH, Secchi, MicrocystisRank) %>% 
          filter(n > 1) %>% select(-n) %>% left_join(fish_sample_cleaned) 

fish_sample_cleaned0 <- fish_sample_cleaned %>%
  mutate(fish_sample_cleaned, Duplicated = (case_when(fish_sample_cleaned$EventID %in% DuplicatedWQ$EventID ~ "duplicated", TRUE ~ "not duplicated")))

fish_sample_cleaned <- fish_sample_cleaned0

filterEventID <- unique(fish_sample_cleaned0$EventID)
```
...
PlusCountFalse = c("2010_LP376", "2011_LP526", "2025_SPLT161", "2024_SHRK222", "2024_SHRK223", "2024_SHRK224")
#20100930_1100_AL3_LP, 20110819_1102_AL4_LP

# These had the wrong count
Count1 = c("2011_LP376", "2025_SPLT161", "2024_SHRK222", "2024_SHRK223", "2024_SHRK224")
...
# These are mistakes
# also removing erroneous shrimp data (found by Nicole)
None = c("2007_NONE1", "2007_NONE6", "2007_NONE7", "2010_NONE3", "2021_SHRK39", "2021_SHRK40", "2021_SHR127", "2017_SHR148", "2021_SHR129", "2007_SHR31", "2009_SHR2")
...
fish_catch_QC2$Dead[fish_catch_QC2$OrganismID == "2024_SHR131"] <- "Yes"

#duplicate removal of seine data removed earlier in code for ease of use in 
#analysis for accuracy - code found in JTs beach seine analysis paper
...
# incorrect mss length BSEIN 2023 (9mm -> should be 44mm per data sheet)
mss <- fish_catch_QC3 %>% 
  filter(EventID == 10320 & OrganismCode == "MSS")
FL_MSS <- "2023_MSS1431"

individuals <-  individuals0 %>%
  mutate(ForkLength = replace(ForkLength, OrganismID %in% FL_NA, NA)) %>% 
  mutate(ForkLength = replace(ForkLength, OrganismID %in% FL_MSS, 44))
```
